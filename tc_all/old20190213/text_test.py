...
def mytest():
	print("Loading test data...")
	t1=time.time()
	x_test,y_test=myprocess_file(config.test_filename,word_to_id,cat_to_id,config.seq_length)
...
if __name__ == '__main__':
	...
	if not os.path.exists(config.vocab_filename):
		...
		build_myvocab_all(config.train_filename, config.vocab_filename, config.vocab_size, config.vector_word_filename)
